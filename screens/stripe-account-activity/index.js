...
const StripeAccountActivity = () => {
  ...
  return <View style={styles.container}>
      ...
      <TabView tabTitles={["All", "All Payouts"]} onPress={setSelectedTab} selected={selectedTab} style={styles.tabView} />
      <FlatList data={history} renderItem={({
      item
    }) => <View style={styles.historyTab}>
            <Text style={styles.heading}>{item.date}</Text>
            {item.list.map((historyItem, index) => <View style={styles.historyItem} key={index}>
                <Image source={historyItem.type === "Debit" ? require("./assets/debitIcon.png") : require("./assets/creditIcon.png")} style={styles.historyIcon} />
                <View style={styles.detailView}>
                  <Text style={styles.heading}>{historyItem.title}</Text>
                  <Text style={styles.subText}>{historyItem.time}</Text>
                </View>
                <View style={styles.balanceView}>
                  <Text style={[styles.subText, historyItem.type === "Debit" ? styles.debit : styles.credit]}>
